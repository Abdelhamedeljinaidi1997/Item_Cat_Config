...
You can visit http://54.93.121.184/ for the website deployed.
...
- Move this private key file named `LightsailDefaultPrivateKey-*.pem` into the local folder `~/.ssh` and rename it `id_rsa`.
- In your terminal, type: `chmod 600 ~/.ssh/id_rsa`.
- To connect to the instance via the terminal: `ssh -i ~/.ssh/id_rsa ubuntu@54.93.121.184`, 
  where `54.93.121.184` is the public IP address of the instance.

<!--
Public IP address is 54.93.121.184.
ssh -i ~/.ssh/id_rsa ubuntu@54.93.121.184
-->
...
- From your local terminal, run: `ssh -i ~/.ssh/id_rsa -p 2200 ubuntu@54.93.121.184`, where `54.93.121.184` is the public IP address of the instance.

<!--
Public IP address is 54.93.121.184
ssh -i ~/.ssh/id_rsa -p 2200 ubuntu@54.93.121.184
-->
...
### Step 5.2: Updated packages to most recent versions
...
- Save and exit using CTRL+X and confirm with Y.
- Verify that `grader` has sudo permissions. Run `su - grader`, enter the password,now you logged in as grader.
...
- On the grader's virtual machine:
  ...
  - Restart SSH: `sudo service ssh restart`
- On the local machine, run: `ssh -i ~/.ssh/grader_key -p 2200 grader@54.93.121.184`.

<!--
Public IP address is 54.93.121.184.
ssh -i ~/.ssh/grader_key -p 2200 grader@54.93.121.184
...
- Save and exit using CTRL+X and confirm with Y.
- Verify that `catalog` has sudo permissions. Run `su - catalog`, enter the password, now you are logged as catalog .
...
`sudo git clone https://github.com/Abdelhamedeljinaidi1997/udacity-catalog-app.git catalog`.
...
- Replace `lig.random_para(250)` by `lig.random_para(100)` on lines 86, 143, 191, 234 and 280.
...
- Open your browser to http://54.93.121.184/
